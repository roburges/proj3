...
def get_user_tweets(user):
	...
	if tweets in CACHE_DICTION: ##LOOP, READ, AND ASSIGN RESULTS VARIABLE FOR CACHE DICTION
		twitter_resultes=CACHE_DICTION[tweets]
...
print('starting function invocation for usertweets')
...
cur.execute("CREATE TABLE IF NOT EXISTS 206_APIsAndDBs")##refresh tables if already exists drop
for tw in umich_tweets:
    ...
    cur.execute('INSERT INTO 206_APIsAndDBs (tweet_id, author, time_posted, tweet_text, retweets VALUES (?,?,?,?,?)', tup)
